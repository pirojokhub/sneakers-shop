...
function OdezhdaBlogPage({ tShorts }) {
	const { id } = useParams(); 
	const productId = parseInt(id, 10);
	const product = tShorts.find((p) => p.id === productId);
		...
		return (
			<section className="single-blog">
				<div className="container">
					...
					>
						Товар не найден!
						<Link to="/odezhda">Вернуться к каталогу</Link>
					</p>
				</div>
			</section>
		);
	}
	const fullContentParagraphs = Array.isArray(product.fullContent)
		? product.fullContent
		: [];
	console.log(fullContentParagraphs);
	return (
		<section className="single-blog">
			<div className="container">
				<div className="single-blog__wrapper">
					<h2 className="single-blog__title">{product.title}</h2>
					<div className="single-blog__date">
						<span>{product.date}</span>
						<span className="single-blog__date_watch">
							<svg
								width="20"
								height="20"
								viewBox="0 0 20 20"
					...
					<div className="single-blog__desp">
						<div className="single-blog__text">
							{fullContentParagraphs.map((paragraph, index) => (
					...
					<Link to={"/blog"}>
						<button className="single-blog__btn">
							Вернуться к секцию блог
						</button>
					</Link>
				</div>
			</div>
...
export default OdezhdaBlogPage;
